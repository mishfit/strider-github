...
function get_github_repos(user, callback) {
  ...
  var team_repos = [];
  var repos = [];
  debug('Fetching Github repositories for user: %s', user.login);
  step(
    function fetchReposAndOrgs() {
      debug('Repos API call for user: %s', user.login);
    ...
    function fetchOrgTeams(err, r, o) {
      ...
      debug('Repos API call returned for user: %s status: %s', user.login, r.response.statusCode);
      debug('Orgs API call returned for user: %s status: %s', user.login, o.response.statusCode);
    ...
    function finalize(err, results) {
      ...
      results.forEach(function (result) {
        if (result && result.data) {
          ...
          debug('get_github_repos(): finalize result was null for user %s', user.login);
        }
      });
      ...
      debug('Github results for user %s - Repos: %j Team Repos w/ admin: %j Org memberships: %j',
        user.login, _.map(repos, 'name'), _.map(team_repos, 'name'),
...
function setup_integration(user_obj, gh_repo_id, token, callback, socket, no_ssh) {
  ...
  step(
    ...
    function push_deploy_key(err, repo_config) {
      ...
      var title = `StriderDeployKey - ${config.strider_server_name} - ${user_obj.login}`;
...
function setup_integration_manual(req, org, github_url, callback) {
  ...
  step(
    ...
    function done() {
      var deploy_key_title = `StriderDeployKey - ${config.strider_server_name} - ${req.user.login}`;
